...
Movement_duration_training_dataset = Night_lightOff_info
Movement_duration_training_dataset_name = 'NF'
...
for i in Movement_duration_training_dataset.index:
    ...
    ExperimentTime = Movement_duration_training_dataset.loc[i,'ExperimentTime']
...
def get_mv_duration(movement_duration_data,time_stage,mv):
    ...
    if len(arr) <= 1 or arr.sum() == 0:
        ...
        #data = (arr,)
        data = arr[:, np.newaxis]
        #data = np.random.choice(arr,size=1000)
        X_plot = np.linspace(data.min(), data.max())[:, np.newaxis]
        kde = KernelDensity(kernel="gaussian", bandwidth=3).fit(data)
        log_dens = kde.score_samples(X_plot)
        dens = np.exp(log_dens)
        dens = dens/np.sum(dens)
        #res = bootstrap(data, np.std, axis=-1, confidence_level=0.95, n_resamples=1000, random_state=rng)
        #ci_l, ci_u = res.confidence_interval
        #res = bootstrap((data,), np.std, confidence_level=0.95,random_state=rng,n_resamples=1000)
        #fig, ax = plt.subplots()
        #ax.plot(X_plot[:, 0], dens, c="#AAAAFF")
        #plt.ioff()
        #n1, bins1, patches1 = plt.hist(data, bins=100, density=True,color='g', alpha=1,)
        #plt.plot(bins1[:-1],n1,'--')

        duration_length = int(rate_random(X_plot[:, 0],dens))
...
#         ci_l, ci_u = res.confidence_interval
#         
#         #duration_rate =  count_duration_rate(ci_l, ci_u,arr)
#     
#         x = np.linspace(ci_l, ci_u)
#         #print(x)
#         #y = np.repeat(1/len(x),len(x))
#         std_sample = np.std(data, ddof = 0)
#         pdf = norm.pdf(x, loc=std_sample, scale=res.standard_error) 
#         #duration_length = int(rate_random(duration_rate['frame_length'].values,duration_rate['probility'].values))
#         duration_length = int(rate_random(x,pdf))
# =============================================================================
...
def main():
    t1 = time.time()
    p = Pool(16)
    p.map(processing_data,range(50))                           ### generate 50 one hour long stimulated movement sequences
    p.close()
    p.join()
    t2 = time.time()
